...
const Survey = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const surveyData = await api.verifyAvailableSurvey();
      if (surveyData.status === 'success' && surveyData.data.length > 0) {
        setProgramData({id: surveyData.data[0].id, name: surveyData.data[0].programName });
      }
    };
    ...
    <div className='w-screen min-h-screen overflow-x-hidden flex justify-center py-4'>
      <div className='card w-5/6 sm:w-4/5 lg:w-1/2 h-fit p-4 bg-base-100 shadow-md'>
        ...
        </div>
        {
          programData.id < 0 ? (
            <>
              <p className='text-xl text-center sm:text-2xl lg:text-4xl font-bold'> Por el momento no hay encuestas disponibles. </p>
            </>
          ) :
            (
              <>
                <p className='text-xl text-center sm:text-2xl lg:text-4xl font-bold'> { strings.titleForm } </p>
                <p className='text-xl text-center sm:text-2xl lg:text-4xl font-bold'> Programa - { programData.name } </p>
